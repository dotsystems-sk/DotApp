...
class DotApper
{
            ...
            switch ($key) {
                ...
                case 'create-controller':
                    if ($this->modul == "") {
                        ...
                        if ($value != "") $this->createController($value);
                        else echo "Specify controller name ! --create-controller=NAME\n\n";
                    }
                    break;
                case 'test':
                    $this->runTests(1);
                ...
                case 'create-middleware':
                    if ($this->modul == "") {
                        ...
                        if ($value != "") $this->createMiddleware($value);
                        else echo "Specify controller name ! --create-controller=NAME\n\n";
                    }
                    break;
                case 'create-model':
                    if ($this->modul == "") {
                        ...
                        if ($value != "") $this->createModel($value);
                        else echo "Specify controller name ! --create-controller=NAME\n\n";
                    }
                    break;
    ...
    function downloadAndUnzip($urlOfFile, $whereToExtract, $overwrite = false, $filesToCopy = null, $filesToSkip = null, $sourceDir = null, $deleteZip = true)
    {
        ...
        if (!$overwrite) {
            ...
            if ($zip->open($zipFile) === true) {
                ...
                for ($i = 0; $i < $zip->numFiles; $i++) {
                    ...
                    if (is_array($filesToSkip) && (in_array($destRealPath, array_map(function ($path) {
                        return realpath($path) ?: $path;
                    }, $filesToSkip)) || in_array($destParentRealPath, array_map(function ($path) {
                        return realpath($path) ?: $path;
                    }, $filesToSkip)))) {
                        continue;
                    }
                    ...
                    if (is_array($filesToCopy) && !empty($filesToCopy)) {
                        if (!in_array($destRealPath, array_map(function ($path) {
                            return realpath($path) ?: $path;
                        }, $filesToCopy)) && !in_array($destParentRealPath, array_map(function ($path) {
                            return realpath($path) ?: $path;
                        }, $filesToCopy))) {
                            continue;
                        }
        ...
        $filesToCopy = is_array($filesToCopy) ? array_map(function ($path) {
            return realpath($path) ?: $path;
        }, $filesToCopy) : null;
        $filesToSkip = is_array($filesToSkip) ? array_map(function ($path) {
            return realpath($path) ?: $path;
        }, $filesToSkip) : null;
    ...
    private function printHelp()
    {
        ...
        echo "  --module=<module_number or module_name> --create-model=ModelName -> Create new model in selected module\n";
        ...
        echo "  --install-module=<git_url|module_name>[:version] -> Install module from git URL or name with optional version\n\n";
    }
