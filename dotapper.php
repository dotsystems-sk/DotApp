...
class DotApper
{
    ...
    /**
     * Constructor — initialize arguments from the command line.
     *
     * @param array $args Arguments from $argv (without the script name)
     */
    public function __construct(array $args)
    {
        $this->args = $args;
        $this->options = []; // Initialize $options in the constructor
        $this->parseArguments();
    }
    ...
    public function run()
    {
        ...
        // Process recognized options left-to-right
        foreach ($this->options as $key => $value) {
            switch ($key) {
                ...
                case 'force':
                case 'github-token':
                    // modifiers handled by the commands above
                    break;
                default:
    ...
    private function runTests($type)
    {
        ...
        // $type = 2 — all modules. $type = 1 — core, or a specific module if one is selected
    ...
    private function installDotapp()
    {
        ...
        // Cheap existence check for DotApp — better than nothing
    ...
    private function listModules()
    {
        ...
        $modules = array_filter(
            ...
            function ($item) use ($modulesPath) {
                // Skip . and .. and keep directories only
    ...
    private function runModuleInstaller(string $moduleName)
    {
        ...
        // 1. Check that ./app/modules/modulename exists
        if (!is_dir($basePath)) {
    ...
    private function createModule(string $moduleName, $i = "")
    {
        ...
        // 1. Check that ./app/modules exists
        if (!is_dir($basePath)) {
            // Recursively create ./app/modules
        ...
        // 2. Check that the module does not already exist
        ...
        $this->createFile($modulePath . "/AI_RULES.md", base64_encode($this->moduleAirulesPointer($moduleName)));
    ...
    /**
     * Short per-module pointer. AIRULES ships with the framework; DotApper does not embed it.
     */
    private function moduleAirulesPointer(string $moduleName): string
    {
        return "# {$moduleName} — AI rules\n"
            . "\n"
            . "If the project root contains an `AIRULES/` folder, follow those rules when creating or changing this module.\n"
     ...
     *
     * @param string $path Directory path (e.g. /foo/bar/baz)
    ...
    {
        // Normalize the path (replace \\ with / and strip extra slashes)
    ...
    /**
     * Create a file from base64-decoded content.
     *
     * @param string $filePath File path (e.g. /foo/bar/file.txt)
     * @param string $base64Content File contents encoded as base64
    ...
    private function printHelp()
    {
        ...
        echo "  --force -> Modifier: overwrite existing files (--install-module)\n\n";
    }
...
$args = $argv;
array_shift($args); // Remove the script name (dotapper.php)
