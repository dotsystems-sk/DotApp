...
6. **Migrations:** after you add a version in `Installation.php`, **MUST** rename `installed_*_install.php` back to `install.php` so the next page load runs it. Do not leave this for the user. Version keys run in **written array order**; append them and **MUST NOT** sort with `ksort`/`krsort`. **MUST** probe then `CREATE`/`ALTER` — **MUST NOT** `CREATE TABLE IF NOT EXISTS` ([07](07-SCHEMA-AND-INSTALL.md) §0).
...
23. **Same class of bug (LAW):** one broken `load()` / `form()` handler means grep this module for siblings; unwrap nested `data`, and use HTTP 200 + `status` for handled product outcomes. See [§2q](#2q-same-class-of-bug--hunt-the-module-must--law).
24. **No unsolicited browser (LAW):** finish gate + source UX + tests + click list by default. Browser only when the user explicitly asks or says yes. See [§2r](#2r-no-unsolicited-browser--click-through-must--law).
...
| **IDs encrypted** | Views / JS / JSON: `value=`, `data-*`, hidden, payload, URL paths | Plain `7` / `{{ var: $id }}` as an id. **MUST** encrypt with a unique `$key2`. A path `{token}` / `href` / `redirectTo` must additionally be sealed to `[A-Za-z0-9_-]+`; raw standard-base64 `%2F` becomes a path separator. Decrypt `=== false` → reject. Still `Auth::can` / ownership in PHP ([11](11-AUTH-AND-CRYPTO.md) §8) |
...
| **Inputs** | Request + persist | Password / HTML / URL / setting / token / hash from `$request->data()` not `data(true)`; missing `form()` / `Validator` where required; persist with only an FE overlay — PHP **MUST** still refuse |
...
| **Layout / UX** | New/moved buttons, cards, forms, footers | Action flush to a parent edge; final button row with no bottom padding; cramped accidental alignment; mobile overflow or hover-only control ([§2f](#2f-layout-and-uxui-must--law)) |
| **AJAX / same class** | Every `load()` / `form()` URL and action in this module | Flat `$request->data()['id']` read although fields are nested under `data`; handled product failure returned as HTTP 400/500; only the reported sibling fixed ([§2q](#2q-same-class-of-bug--hunt-the-module-must--law)) |
...
| **Installer order** | `Installation.php` / installer maps | `ksort`, `uksort`, `krsort`, or `usort` on installer/uninstaller keys; new version inserted before an older dependency instead of appended in written order ([07](07-SCHEMA-AND-INSTALL.md)) |
...
| **Browser discipline** | Verification actions | Browser/CDP/screenshot click-through used without the user explicitly asking or saying yes ([§2r](#2r-no-unsolicited-browser--click-through-must--law)) |
...
A working control that looks unfinished is a bug. Every visible chunk must work on desktop and mobile.

When adding an action control: check padding on **all sides**, especially below when it is the last content in a card, modal, drawer, or page section. It **MUST NOT** sit flush against the parent. A footer with `pt-0` still needs compensating bottom space. Align the cluster deliberately with sibling content; provide an adequate touch target and wrapping without overflow.

Fail now for a Save/primary glued to an edge, a final button row with no bottom padding, a cramped/accidentally left-shoved cluster, or hover-only mobile placement. Canonical: [05](05-VIEWS-TEMPLATES-ASSETS.md) §8c.
...
`$dotapp().load(url, "POST", payload)` and secure `form()` submissions post fields under `data`, plus a top-level CRC. PHP that reads `$request->data()['id']` as a flat payload misses the field. A product failure returned as HTTP 400/500 enters `load()`’s transport-error callback, whose arguments are `(httpStatus, bodyText)`, so `reply.message` is usually replaced by generic “Request failed”.

**MUST:** unwrap the posted/form bag; use HTTP **200** + `status` 0|1 + `message` for handled product outcomes. Keep transport/auth failures as transport codes (for example CRC 400 and login/rights 403). When one broken handler reveals a shared class — nesting, wrong HTTP outcome, missing toast, CRC duplication, escaping — grep **this module** and fix every sibling in the same chunk. **MUST NOT** fix only Disable while Delete/Test/Clear retain the same bug.

Canonical: [08](08-FORMS-AND-SECURITY.md), [09](09-DOTAPP-JS-AND-BRIDGE.md) §3. Compact: `AIRULES/cursor/rules/24-dotapp-ajax-and-class-bugs.mdc`.

### 2r. No unsolicited browser / click-through (MUST — law)

Default verification is source inspection, the finish gate, tests, and a short operator click list. **MUST NOT** open browser tools, CDP, screenshot-and-click, or browser subagents to prove Save/Delete/routing unless the user explicitly commands browser verification in this turn or answers yes to a focused ask. No answer means no browser.

When the user reports a live failure, follow [23](23-DEBUG-PLAYBOOK.md) and grep first. Browser access is still opt-in. Compact: `AIRULES/cursor/rules/25-no-unsolicited-browser.mdc`.
...
4. **MUST encrypt every identifier sent to the browser** (`<option value>`, `data-*`, hidden, JSON). Use `{{ enc(Shop.user.id): $id }}` / `Crypto::encrypt($id, 'Shop.user.id')` with a **different `$key2` per field**. Never `value="7"` / `data-id="7"`. A URL path token **MUST additionally** seal standard base64 to `[A-Za-z0-9_-]+` and accept sealed + legacy standard forms when opening; raw `%2F` is treated as a slash by Apache. Decrypt with the same `$key2`; `false` → reject. **MUST still** `Auth::can()` / ownership ([11](11-AUTH-AND-CRYPTO.md) §8).
...
12. **MUST** take every value that is stored, hashed, decrypted, or compared as typed from `$request->data(true)` / `$request->query(true)` (passwords, HTML, URLs, settings, tokens, signatures, ids, titles, notes). `$request->data()` is the **protected** copy (`protect()`); it rewrites `=` and other characters and **MUST NOT** be persisted. Bind original strings in SQL and escape on output. **MUST** show every login failure. Canonical: [19](19-VALIDATION-AND-INPUT.md).
...
32. **Installer key order (MUST):** `installer()` / `uninstaller()` keys run in written PHP array order. **MUST NOT** sort the maps with `ksort`, `uksort`, `krsort`, or `usort`; uninstall reverses written order. Canonical: [07](07-SCHEMA-AND-INSTALL.md).
33. **AJAX product outcomes (MUST):** unwrap `load()` / `form()` fields from nested `data`; handled product success/failure returns HTTP 200 + `status` 1|0 + `message`. Transport/auth failures retain their real status. One hit requires a same-class hunt in this module. Canonical: [§2q](#2q-same-class-of-bug--hunt-the-module-must--law), [09](09-DOTAPP-JS-AND-BRIDGE.md) §3.
