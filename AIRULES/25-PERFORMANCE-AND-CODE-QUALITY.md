...
**Budget per screen:** a list = **one** query (`paginate()` already returns the total). A detail = **one** query, plus one per related list. Anything more needs a reason in a comment.

| Need | Do (**MUST**) | **MUST NOT** |
|------|---------------|--------------|
| Does it exist? | `exists()` | `->all()` then `count($rows)` |
| How many? | `select('COUNT(*) as total')` or `paginate()['total']` | fetch rows to count them |
...
| Index (or cover with a composite) every column in `WHERE`, `JOIN`, `ORDER BY` on a table that grows | otherwise every page scans the table |
...
| Adding an index later = a **new version** in `Installation.php` + `alterTable` guarded by `indexExists()` | migrations are the only way schema changes reach existing installs ([07](07-SCHEMA-AND-INSTALL.md)) |
...
| Rule (**MUST**) | **MUST NOT** |
|-----------------|--------------|
| Tables named `{lowercase_modulename}_*` ([07](07-SCHEMA-AND-INSTALL.md)) | unprefixed / `dotapp_*` names for module data |
...
- `COUNT(*)` over a huge table is not free — reuse `paginate()['total']` instead of a second count, and for very large log tables consider counting only on the first page.
- Send back **only what changed**: patch the affected rows + the pager from JSON ([09](09-DOTAPP-JS-AND-BRIDGE.md) §3). **MUST NOT** re-render the whole table (or reload the page) after one toggle.
...
| Rule (**MUST**) | **MUST NOT** |
|-----------------|--------------|
| One CSS + one JS per module, self-hosted in that module’s `assets/` | a file per widget, or a CDN `<script>` ([24](24-ATTACK-VECTORS.md) §10) |
...
**Style:** English comments; `StudlyCase` classes, `camelCase` methods/variables, `snake_case` DB columns; follow the surrounding module’s existing formatting rather than introducing a new one. User-visible strings stay product copy ([05](05-VIEWS-TEMPLATES-ASSETS.md) §8).
