...
Every row below is a **law**: that attack **MUST NOT** be possible in code you ship. This file is the **catalogue** (what must never happen + the DotApp mechanism). The long *how* stays in the linked doc — do not duplicate it here.

**How to use it (cheap):** open only the sections for the surface you touch. Writing SQL → §1. Login / 2FA → §3. A view or JSON → §2, §5. Upload → §6. Public endpoint → §7. Then run the **threat pass** in §11 on your diff.
...
2. **The server decides.** Hidden fields, disabled buttons, an omitted link, JS validation, an overlay = **UX**. Every gate **MUST** also exist in PHP. Removing the frontend **MUST** still fail ([08](08-FORMS-AND-SECURITY.md), [11](11-AUTH-AND-CRYPTO.md) §11).
3. **Not listed is not allowed.** If a vector is missing from this file, apply the nearest row **and say so in chat**. “Probably fine” is a bug. Never weaken a rule to make a feature work — **ask** instead.
...
| Open redirect | Redirect targets from a fixed allowlist or your own path. Never `Response::redirect($request->query(true)['next'])` | [03](03-MODULES-AND-ROUTING.md) |
...
| Mass assignment | Insert / update an **explicit column whitelist**. Never spread `$request->data()` into the query — a posted `is_admin`, `user_id`, `price`, or `status` must be ignored | [06](06-DATABASE.md) |
...
| Attack | Law (**MUST**) | Where |
|--------|----------------|-------|
| CSRF | `crcCheck()` **exactly once** per state-changing POST (API prefix **XOR** action — two calls burn the token) | [08](08-FORMS-AND-SECURITY.md) |
...
| Remember-me 2FA skip | The RM path skips the 2FA stage — **MUST NOT** enable `rm_autologin` for a 2FA-protected area | [11](11-AUTH-AND-CRYPTO.md) §6 |
| Auth bypass by route | Login-only handlers registered **only** inside `if (Auth::isLogged() === true)`, HTML subtree covered by a `Gate@login` `before` returning 403 | [03](03-MODULES-AND-ROUTING.md) |
...
| Missing function-level check | `Auth::can()` on **every** action, not only where the view hides a button | [11](11-AUTH-AND-CRYPTO.md) §3 |
| Privilege escalation | **MUST NOT** grant a right the actor lacks, mutate a more privileged account, or promote a target into a tier the actor is not | [11](11-AUTH-AND-CRYPTO.md) §11 |
...
| Client-side authorization | A hidden tab / disabled control is UX. The endpoint refuses on its own | [08](08-FORMS-AND-SECURITY.md) |
...
| Attack | Law (**MUST**) | Where |
|--------|----------------|-------|
| Clickjacking | `X-Frame-Options: SAMEORIGIN` (or CSP `frame-ancestors`) on authenticated / form pages | — |
...
| Cached private page | Authenticated HTML / JSON: `Cache-Control: no-store` | — |
...
| Data over-fetch in JSON | `select` only the columns the screen needs — never dump a user/order row (hashes, tokens, internal flags) into `ajaxReply` | [06](06-DATABASE.md) |
...
| Paid-API drain (AI, SMS, mail) | Login + rights + rate limit before any paid call; cap payload size | [22](22-AI-SEARCH-MCP.md) |
...
| Attack | Law (**MUST**) | Where |
|--------|----------------|-------|
| CDN / remote script | Self-host libraries in **your** module assets and pin the version. No `<script src="https://…">` on authenticated pages | [05](05-VIEWS-TEMPLATES-ASSETS.md) |
...
| AI acting beyond the actor | An AI tool that writes **MUST** run the same `Auth::can` + owner scope as the manual path | [22](22-AI-SEARCH-MCP.md) |
...
| 12 | the route list | a public `noauth` endpoint shipped **without the bot warning in chat**; a registered URL with no handler |
