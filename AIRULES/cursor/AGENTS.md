...
11. **Extender** — judge first, **not** every method. Owner `exists()` + immediate `return call()`; extender `extend()` in `Listeners::register()` before Module initialization. Target URLs in listener map; own Module routes or `[]`; controller string preferred. Not `.loaded` for initialize-time, Events, or `$request`/secrets. Canonical: `AIRULES/12-SERVICES.md` §10, `AIRULES/00-AGENT-CONTRACT.md` §2h.
...
- **Extender (judge — not every method):** owner `exists()` + immediate `return call()`; `extend()` in `Listeners::register()`; target URLs in explicit listener routes; Module owns only its routes or `[]`; prefer a controller string. **MUST NOT** use listener `['*']` just to attach or `.loaded` for initialize-time. Canonical: `AIRULES/12-SERVICES.md` §10, EX-17.
