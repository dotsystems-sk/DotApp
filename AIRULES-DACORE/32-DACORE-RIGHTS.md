...
### Step-up 2FA — ASK, do not invent (**MUST**)

Login 2FA already happened. A second prompt is extra friction. **MUST NOT** add it to every settings Save, theme, label, catalog CRUD, list filter, or “to be safe.”

**Plan (MUST ASK once):** “Which actions, if any, need a fresh 2FA code after login?”

| Answer | What you ship |
|--------|----------------|
| User names actions | Step-up **only** on those actions |
| User says none / no answer | **No** step-up. Rights + CRC + PHP checks are enough |
| Agent invents it on ordinary Save | **Bug** |

Typical **yes** (examples — not a default list to implement): install/uninstall a package, restore or wipe data, grant `dotapp.root`, change a filesystem jail, turn off a global security flag. Installation is the kind of action that **may** deserve a code — ordinary settings do not.

DACore’s plugin installer already uses this chrome. Your module copies that **look** only when the user asked. Pattern: [EX-D10](examples/EX-D10-stepup-2fa-modal.md).

### When step-up exists: one chrome (**MUST**)

Canonical (read-only): `app/modules/DACore/views/layouts/plugins/_gate.layout.php` + `plugins-admin.js` (`autoSubmit: true`).

**MUST:**

- Centered Bootstrap modal (`$dotapp(el).modal()`), static backdrop
- Title + one sentence why this action needs a code
- Method hint (authenticator / email / SMS)
- **Send confirmation code** only when the method is email/SMS
- Six square boxes: `dacore-otp two-fa-inputs`, `type="password"`, `inputmode="numeric"`, `maxlength="1"`; first `autocomplete="one-time-code"`
- Load DACore OTP sizing: `/assets/modules/DACore/css/pages/dotapp-ui/users.css` on `withMenu` `$css` (consume — do not fork `users.css` into your module)
- `$dotapp("#… input").twoFactor(submitFn, { length: 6, autoSubmit: true, allowLetters: false, invalidClass: "is-invalid" })`
- Paste of six digits **or** the sixth box filled → **immediately** POST (`form` / `load`). Confirm is fallback only
- Overlay `.modal-content` until the request ends ([09](09-DOTAPP-JS-AND-BRIDGE.md) §3)
- Footer padding vs the modal (`pb-4`). A lock-in gate may omit Cancel; an optional confirm **MUST** keep Cancel

**MUST NOT:** one `<input maxlength="6">` on the settings card; OTP boxes inline under Save; a custom OTP / jQuery widget; `autoSubmit: false` on unlock/confirm step-up; a second look per module; `Auth::confirmTwoFactor` (login stage 2 only).

**Which code:** the method that is **on** for this user. Prefer authenticator if `tfa_auth = 1`; else SMS; else email. SMS/email codes are generated by **you** and **you** send them ([21](21-EMAIL-SMS-QR.md)) — core does not send.
...
Still `Auth::can()` / encrypted ids / `crcCheck()`.

The overlay is **UX only**. **MUST** verify the 2FA code in PHP **before** persisting. If the request skipped the modal (overlay removed, Save clicked anyway, DevTools POST), **MUST NOT** write. No code / wrong code → refuse. See [08](08-FORMS-AND-SECURITY.md) “Server is the authority”.

**Dangerous flags (only if the plan named step-up on that flag):** do not persist “off” on the same handler as ordinary settings. The general save **MUST ignore** a request that turns the flag off (including a crafted POST). Turning it off **MUST** be a separate PHP handler that verifies a 2FA code first. Covering Save with a 2FA overlay is not enough.
...
| `Auth::confirmTwoFactor` for a logged-in step-up | TOTP: `TOTP::generate` vs `attributes()['tfa_auth_secret']`; SMS/email: your one-time challenge (this file §6) |
| Step-up on every settings Save without the user asking | **ASK** in the plan; default **no** (this file §6) |
...
| Named dangerous flag turned off on the same save as other settings | General save ignores “off”; a separate 2FA handler turns it off (this file §6) |
