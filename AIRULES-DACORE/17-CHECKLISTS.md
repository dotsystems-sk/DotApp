...
- [ ] **Extender (judge):** owner `exists()` + immediate `return call()`; `extend()` in `Listeners::register()`; target listener routes explicit; own Module routes or `[]`; controller string preferred; no `['*']` just to attach; no `.loaded` for initialize-time; no `$request`/secrets, Events, duplicate, owner/DACore patch ([12](12-SERVICES.md) §10, [00](00-AGENT-CONTRACT.md) §2h)
