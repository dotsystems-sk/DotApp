...
## defaultSettings before routes (**MUST** — law)

**MUST** put Config defaults in `defaultSettings()` and call it at the start of `initializeRoutes()` (before `return`) **and** at the start of `initialize()`. **MUST NOT** compose wake lists or `Router` paths from Config that another module fills only later (DACore `prefixUrl`). **MUST NOT** hardcode that module’s fallback so their `defaultSettings()` never runs. Use a literal path owned by the current module if the dependency has not run yet. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2m, `AIRULES/03-MODULES-AND-ROUTING.md`.

## Module AIRULES (**MUST** — law)

A host that other modules extend **MUST** keep `app/modules/<Host>/AIRULES/`. When that folder exists on a **named** host, follow **project `AIRULES/` + those files together**. Module rules add host routes and contracts; they **MUST NOT** weaken project law. **MUST NOT** open `app/modules/<Host>/PLAN/` when writing `<Pack>`. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2n, §2p.
...
2. **IDs** — no plain `value="7"` / `data-id="7"` / `{{ var: $id }}` as an id. **MUST** `{{ enc(Shop.item.id): $id }}` unique `$key2` for form/`data-*`. Path `{token}` / `redirectTo` **MUST** be sealed `[A-Za-z0-9_-]+` (Apache 404 on `%2F`). Decrypt `false` → reject. Still `Auth::can` / ownership in PHP.
3. **Queries** — bindings only. No user input in SQL. `$qb->raw()`: every `?` is a placeholder (comments count).
4. **Inputs** — persist / passwords / HTML / URLs / hashes from `$request->data(true)`. SQL write = named / `?` bindings, not `protect()` (`=` → `&#61;`). Persist re-checked in PHP (if the plan named step-up, PHP still refuses without a valid code). FE overlay is UX only.
...
10. **Layout / UX-UI** — every new button has padding vs the parent on **all sides** (left, right, top, bottom). When buttons are the **last** content of a card/`card-body`/modal, **almost always** pad **below**. Not flush to the card/page edge; centered or aligned to siblings. `pt-0` footers still have `pb-*` / CSS `padding-bottom`. New/rewritten admin forms **MUST** be numbered sections + one-sentence lede + tinted Why-this-matters — **not** one wall of inputs. Chrome stays in **this** module; **MUST NOT** copy sibling-module chrome. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2f, `AIRULES/33-DACORE-PAGES-AND-UI.md` §6–§6b.
...
12. **DACore is not Bootstrap** — grep `data-bs-toggle="tab"` / `.tab-pane.fade` / `bootstrap.bundle` in this module. Tab look = real GET links + this module’s CSS. Grid = `<dot-grid>` / `<dot-col>`. Canonical: `AIRULES/33-DACORE-PAGES-AND-UI.md` §0.
13. **Hooks** — fire **`module.{lowercase_modulename}.{hook_name}.hook`** only when another module could log, show history, or sync (SMS/mail sent, payment, lockout) — **MUST NOT** on every save. Above `trigger()`: `Hook:` / `Why:` / `About:` / `Params:` / `Use:`. Same name in `app/modules/<ThisModule>/.hooks`. No secrets. No `trigger()` inside `foreach` of a growing list. DACore-bound: read `app/modules/DACore/.hooks` first. Canonical: `AIRULES/41-MODULE-HOOKS.md` §6.
14. **Extender** — judge first, not every method. Owner `exists()` + `call()`; ordinary result returns, only `isOriginal()` continues owner logic. Extender `extend()` belongs in `Listeners::register()` before Module initialization. Target listener routes; own Module routes or `[]`; controller string preferred. No `next()`, marker response, `.loaded` for initialize-time, Events, `$request`/secrets, or DACore patch. Canonical: `AIRULES/12-SERVICES.md` §10.
15. **PHP 7.4+** — unless the plan named a higher version: no `match`, `?->`, union/`mixed`, named args, constructor promotion, attributes, `enum`, `readonly`, `str_contains` / `str_starts_with` / `str_ends_with`. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2i.
16. **Read scope** — tool paths stayed in `<Target>` + DACore (+ a sibling **only** if the user named it as the extend/listen/Extender target). No copy of a sibling’s cards/CSS/chrome. Examples from `AIRULES/examples/`, not a live neighbour. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §1b.
17. **MySQL-safe DDL** — `Installation.php` / `ensureTable` has no `CREATE TABLE IF NOT EXISTS` / `ADD COLUMN IF NOT EXISTS` / `ADD INDEX IF NOT EXISTS`. Probe then `CREATE`/`ALTER`. Canonical: `AIRULES/07-SCHEMA-AND-INSTALL.md` §0.
18. **defaultSettings / routes** — `defaultSettings()` exists; called at the start of `initializeRoutes()` (before `return`) and at the start of `initialize()`. Wake/`Router` paths are not built from Config another module fills later. No hardcoded foreign fallback. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2m.
19. **Module AIRULES** — named host `app/modules/<Host>/AIRULES/` was read when present; pack routes match what the host listens to; a new host that others extend has that folder. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2n.
20. **PLAN folder** — new module / rewrite has `app/modules/<This>/PLAN/` (split files); code matches PLAN; unread PLAN = fail. Pack-specific phases follow `app/modules/<Host>/AIRULES/` when present. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2o.
21. **Rule stack** — host `PLAN/` was **not** opened while writing a pack; project AIRULES was not skipped. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2p.
22. **Admin AJAX** — every `load()` / `form()` unwraps nested `data` and answers HTTP **200** + `status` 0|1. HTTP 400/500 hides `reply.message` as Request failed. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2q, `AIRULES/09-DOTAPP-JS-AND-BRIDGE.md` §3.
23. **Same class of bug** — one broken handler is not enough. Grep **this module** and fix every sibling in the same chunk. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2q.
24. **No unsolicited browser** — **MUST NOT** MCP/CDP/screenshot-click to prove Save/Delete. Finish-gate + source UX + a click list. Browser only if the user asked or said yes. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2r.
25. **Renderer lifecycle** — `dotapp.renderer.before/after` listeners receive `RendererLifecycleContext`; replacement is `useReplacement()` only in `before` for layout/view/rendered code. Listener returns do nothing; custom/after are observe-only; never log output/secrets. Canonical: `AIRULES/05-VIEWS-TEMPLATES-ASSETS.md` §5.
...
General UX/UI principles **MUST** be followed **at all costs**. After adding a button: check **padding vs the parent on all sides** (left, right, top, bottom). When the buttons are the **last content** of a card, `card-body`, modal, or page block, **almost always** add space **below** (`card-footer` + `pb-3`/`pb-4`). Place the cluster deliberately (centered or same rhythm as siblings). A Save glued to the card edge is a **bug**. Admin forms **MUST** use numbered sections, a one-sentence lede, and a tinted Why-this-matters note when the cluster is not obvious — **not** one undifferentiated page of inputs. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2f, `AIRULES/05-VIEWS-TEMPLATES-ASSETS.md` §8c–§8e, `AIRULES/33-DACORE-PAGES-AND-UI.md` §6–§6b.
...
- **PLAN folder (MUST):** write `app/modules/<This>/PLAN/` as a split folder then implement from it. Chat-only / one-file plans fail. **MUST NOT** read `app/modules/<Host>/PLAN/` when writing `<Pack>`; follow the named host’s `AIRULES/` for any host-specific phase. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2o, §2p.
...
- **Same class of bug (MUST):** one broken `load()` / `form()` (nested `data`, HTTP 400 → Request failed) → grep this module and fix every sibling. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2q, `AIRULES/09-DOTAPP-JS-AND-BRIDGE.md` §3.
- **No unsolicited browser (MUST):** do not live-click admin/public pages unless the user asked or said yes. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2r.
- **Renderer lifecycle:** `dotapp.renderer.before/after` use `RendererLifecycleContext`; call `useReplacement()` instead of returning HTML; explicit listener routes; no output/secrets in logs. Canonical: `AIRULES/05-VIEWS-TEMPLATES-ASSETS.md` §5, `AIRULES/examples/EX-18-renderer-lifecycle.md`.
...
- **Request MUST:** `$request->data(true)` / `$request->query(true)` = original. `$request->data()` is **protected** (`protect()`). **MUST** persist original (URLs, settings, HTML, passwords, hashes). SQL injection protection is named / `?` bindings — **MUST NOT** store `protect()`. **MUST** show every login failure. Canonical: `AIRULES/19-VALIDATION-AND-INPUT.md`, `AIRULES/06-DATABASE.md`.
...
| **Same class of bug / admin AJAX** | `AIRULES/00-AGENT-CONTRACT.md` §2q + `AIRULES/09-DOTAPP-JS-AND-BRIDGE.md` §3 — unwrap nested `data`; HTTP 200 + `status`; one hit → hunt this module |
| **No unsolicited browser** | `AIRULES/00-AGENT-CONTRACT.md` §2r — no live click-through unless the user asked; source UX + click list |
...
| **Module AIRULES (host + pack)** | `AIRULES/00-AGENT-CONTRACT.md` §2n — then `app/modules/<Host>/AIRULES/`. **Not** `app/modules/<Host>/PLAN/` |
...
- **DACore is not Bootstrap:** the shell does **not** load `bootstrap.bundle.js`. **MUST NOT** `data-bs-toggle="tab"` (those clicks do nothing). Tab look = real GET subpages + **your** CSS. Grid = `<dot-grid>` / `<dot-col>`. Canonical: `AIRULES/33-DACORE-PAGES-AND-UI.md` §0.
