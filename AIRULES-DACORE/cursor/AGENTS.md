...
12. **Extender** — judge first, not every method. Owner `exists()` + `call()`; ordinary result returns, only `isOriginal()` continues owner logic. Extender `extend()` belongs in `Listeners::register()` before Module initialization. Target listener routes; own Module routes or `[]`; controller string preferred. No `next()`, marker response, `.loaded` for initialize-time, Events, `$request`/secrets, or DACore patch. Canonical: `AIRULES/12-SERVICES.md` §10.
13. **PHP 7.4+** — unless the plan named a higher version: no `match`, `?->`, union/`mixed`, named args, constructor promotion, attributes, `enum`, `readonly`, `str_contains` / `str_starts_with` / `str_ends_with`. Canonical: `AIRULES/00-AGENT-CONTRACT.md` §2i.
