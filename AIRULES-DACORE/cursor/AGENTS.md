...
12. **Extender** — judge first, not every method. Owner `exists()` + immediate `return call()`; extender `extend()` in `Listeners::register()` before Module initialization. Target listener routes; own Module routes or `[]`; controller string preferred. Not `.loaded` for initialize-time, Events, `$request`/secrets, or DACore patch. Canonical: `AIRULES/12-SERVICES.md` §10.
...
- **Extender:** `extend()` in `Listeners::register()`; target URLs in explicit listener routes; own Module routes or `[]`; controller string preferred. No listener `['*']` just to attach, `.loaded` for initialize-time, duplicate, or DACore patch. Canonical: `AIRULES/12-SERVICES.md` §10, EX-17.
