...
**MUST:** users, logs, items, or any accumulating table use COUNT + LIMIT on **first ship** — not `->all()`, not “few rows now so skip”. The pager in the browser is [40](../40-DACORE-LIST-PAGER.md) / [EX-D08](EX-D08-list-pager.md). Lookup lists **MUST** AJAX search unless declined.
