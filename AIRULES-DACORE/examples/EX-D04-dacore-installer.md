...
**Quoted keys (MUST):** every `installer()` / `uninstaller()` key is `'1.0.0' =>` / `"1.0.0" =>` written as text in the file. **MUST NOT** `self::…` / `static::…` / constants / variables as a key — DACore greps `Installation.php` as text and rejects a zip with no quoted keys. Keep constants **inside** the callback.
    ...
    public static function installer()
    {
        return [
            ...
            '1.0.0' => function () {
                ...
                // ---------- 4. menu (ASK: shared nested vs module-own — [31]) ----------
