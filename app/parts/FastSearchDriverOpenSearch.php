...
/**
 * OpenSearch Driver for FastSearch
 *
 * Provides a unified interface for search operations within the DotApp framework,
 * supporting indexing, searching, and document management with OpenSearch.
 *
...
class FastSearchDriverOpenSearch {
    private $host;
    private $auth;
    ...
    public function __construct() {
        $this->host = Config::searchEngines('opensearch_host') ?? 'https://localhost:9200';
        $this->auth = [
            'username' => Config::searchEngines('opensearch_username') ?? '',
            'password' => Config::searchEngines('opensearch_password') ?? '',
            'ca_file' => Config::searchEngines('opensearch_ca_file') ?? '',
            'ca_fingerprint' => Config::searchEngines('opensearch_ca_fingerprint') ?? ''
        ];
        $this->retryAttempts = Config::searchEngines('opensearch_retry_attempts') ?? 3;
        $this->retryDelayMs = Config::searchEngines('opensearch_retry_delay_ms') ?? 200;
    }

    /**
     * Maps OpenSearch error types to FastSearch error codes.
     *
     * @param array $response HTTP response from OpenSearch
     * @param string $operation Operation that failed
     * @return FastSearchError
     */
    private function mapOpenSearchError(array $response, string $operation): FastSearchError {
        ...
        // Handle cURL connection errors (e.g., timeout, connection refused)
     ...
     * @param string|null $rawBody Raw body for bulk operations (e.g., NDJSON)
    ...
    private function executeWithRetry(
        ...
        $retryableCodes = [408, 429, 503]; // Request Timeout, Too Many Requests, Service Unavailable
        ...
        while ($attempts < $maxAttempts) {
            ...
            // Map OpenSearch error
            $error = $this->mapOpenSearchError($response, strtolower($method) . '_request');
    ...
    private function getDriver(): array {
        ...
        $driver['configureIndex'] = function (string $index, $fields, FastSearch $fs): array {
            ...
            if ($response['error'] && $response['error']->getErrorCode() === 'INDEX_NOT_FOUND') {
                ...
                return $createResponse; // Return specific error
            }
            return $response; // Return specific error (e.g., CONNECTION, AUTHENTICATION_FAILED)
        };
        ...
        $driver['deleteIndex'] = function (string $index, FastSearch $fs): array {
            ...
            return $response; // Return specific error
        };
        ...
        $driver['index'] = function (string $index, string $id, array $document, FastSearch $fs): array {
            ...
            return $response; // Return specific error (e.g., INDEX_NOT_FOUND, INVALID_PROPERTIES)
        };
        ...
        /**
         * Normalizes search options for OpenSearch.
         */
        ...
        $driver['update'] = function (string $index, string $id, array $document, FastSearch $fs): array {
            ...
            return $response; // Return specific error (e.g., INDEX_NOT_FOUND, DOCUMENT_NOT_FOUND)
        };
        ...
        $driver['delete'] = function (string $index, string $id, FastSearch $fs): array {
            ...
            return $response; // Return specific error (e.g., INDEX_NOT_FOUND)
        };
