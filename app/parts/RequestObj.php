...
class RequestObj {
    ...
    */
    ...
    public function crcCheck() {
        ...
        if ($this->hasListener("dotapp.invalidate.csrf.url.token")) {
            return $this->dotApp->trigger("dotapp.invalidate.csrf.url.token", $this);
        } else {
            $data = json_decode($data, true) ?? $data; // Pokus o dekódovanie JSON, ak je data reťazec
            if (!is_array($data)) {
                if ($this->dotApp->isDebugMode()) {
    ...
    public function formSignatureCheck() {
        ...
        $data = $this->data();
        $this->dotApp->unprotect($data['dotapp-secure-auto-fnname']);
        $this->dotApp->unprotect($data['dotapp-secure-auto-fnname-action']);
        $this->dotApp->unprotect($data['dotapp-secure-auto-fnname-method']);
        $this->dotApp->unprotect($data['dotapp-secure-auto-fnname-public']);
